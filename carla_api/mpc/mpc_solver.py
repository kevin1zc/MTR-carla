...
class MpcController:
    def __init__(self, world, ego_vehicle, predicted_trajs, waypoints, destination, horizon=N, dt=dt):
        self.world = world
        ...
        self.X = None
        self.U = None
        self.control_buffer = {"acceleration": [
            0] * self.horizon, "wheel_angle": [0] * self.horizon}
        self.buffer_index = 0
        ...
        self.dyn_predicted_traj = np.array(
            predicted_trajs[10:]).reshape(-1, 10, 2)  # (7, 10, 2)
        self.ego_vehicle = ego_vehicle
        self.red_light = [None]*N  # if red_light then True, else False
        self.stop_sign = [None]*N  # if stop_sign then True, else False
        self.waypoints = waypoints if len(waypoints) >= 10 else waypoints + [
            waypoints[-1]] * (10 - len(waypoints))  # next k closest waypoints
    ...
    def reset_solver(self):
        ...
        self.X = self.opti.variable(
            4, self.horizon + 1)  # State: (x, y, yaw, v)
        ...
        for k in range(self.horizon):
            ...
            self.opti.subject_to(
                self.X[:, k + 1] == bicycle_model(self.X[:, k], self.U[:, k], self.dt))
    ...
    def update_cost_function(self, destination=None, obstacles=None):
        ...
        terminal_cost = TERMINAL_COST_COEF * \
            calculate_terminal_deviation(
                self.X[:2, self.horizon-1], destination)
        ...
        for k in range(self.horizon):
            ...
            dyn_obs_collision_cost = DOC_COST_COEF * \
                calculate_min_distance_do(
                    self.X[:2, k], self.dyn_predicted_traj[:, k, :])
    ...
    def run_mpc(self):
        ...
        self.update_cost_function(
            self.get_destination(), self.get_static_obstacles())
        self.solve()
        ...
        print(
            f"Calculation time of MPC controller: {mpc_calculation_time:.6f} seconds")
            ...
            for object_type in self.static_obstacles:
                ...
                for bb in type_bbs:
                    distance = np.linalg.norm(np.array(
                        [self.ego_predicted_traj[k, 0] - bb.location.x, self.ego_predicted_traj[k, 1] - bb.location.y]))
    ...
    # We will not apply control values here. Zhuochen will add this to his code.
